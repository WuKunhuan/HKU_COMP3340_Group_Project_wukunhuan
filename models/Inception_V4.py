...
class Inception_V4(nn.Module):
    ...
    def __init__(self, num_classes, in_channels=3, k=192, l=224, m=256, n=384):
    ...
    def forward(self, x):
        ...
        if (x.size()[1:4] != torch.Size([3, 224, 224])): 
            raise Exception(f"Error: Inception_V4 input dimension expected torch.Size([3, 224, 224]), but {x.size()[-3:0:1]} received instead. ")
