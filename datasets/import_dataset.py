...
class import_dataset: 
    ...
    def __init__(self, setup): 
        ...
        if (len(all_dataset) == 0): 
            ...
            print ("You have not created any dataset! Please create at least 1 dataset(s) above to continue. ")
        else: 
            dataset_select = None
            ...
            dataset = all_dataset[dataset_select]
            self.path = f"{self.root}/datasets/{dataset}"
            ...
            while (not flag): 
                clear_output(wait=True) 
                input (f"Change dataset image transformation in Pytorch in: \n\"{self.root}/datasets/dataset_train_transform.txt\", \n\"{self.root}/datasets/dataset_val_transform.txt\" and \n\"{self.root}/datasets/dataset_test_transform.txt\". \n(After you finished, enter anything to continue)")

                file_read = open(f"{self.root}/datasets/dataset_train_transform.txt", "r"); train_transform = ""; 
                for line in file_read.readlines(): 
                    train_transform += line
                file_read.close(); 

                file_read = open(f"{self.root}/datasets/dataset_val_transform.txt", "r"); val_transform = ""; 
                for line in file_read.readlines(): 
                    val_transform += line
                file_read.close(); 

                file_read = open(f"{self.root}/datasets/dataset_test_transform.txt", "r"); test_transform = ""; 
                for line in file_read.readlines(): 
                    test_transform += line
                file_read.close(); 
                ...
                if (flag == ""): 
                    flag = True
                    self.train_dataset = torchvision.datasets.ImageFolder(root=self.train_path, transform=exec(train_transform))
                    self.val_dataset = torchvision.datasets.ImageFolder(root=self.val_path, transform=exec(val_transform))
                    self.test_dataset = torchvision.datasets.ImageFolder(root=self.test_path, transform=exec(test_transform))
                else:
                    flag = False
            clear_output(wait=True)
